...
function importFromTakeout() {
  ...
  const files = DriveApp.getFilesByName('Saved Places.json');
  if (!files.hasNext()) {
    Browser.msgBox('Saved Places.json がGoogle Driveに見つかりません。\nGoogle TakeoutでエクスポートしたファイルをDriveにアップロードしてください。');
    return;
  }

  const json = JSON.parse(files.next().getContentText());
  ...
  features.forEach(feature => {
    ...
    const title    = props.Title || location['Business Name'] || '';
